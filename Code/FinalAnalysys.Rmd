...
# cv <- xgb.cv(data = dtrain, nrounds = 50, nthread = 2, nfold = 5, metrics = list("rmse"),
#                   max_depth = 3, eta = 0.3, subsample = 0.5, objective = "reg:squarederror", prediction = T, print_every_n = 10)
# #print(cv$evaluation_log)
# #print(cv, verbose=TRUE)
# 
# 
# perf <- sapply(cell_names, function(cn)
#   {
#   pred <- cv$pred
#   cor(pred[model_data$train$cell_name==cn], model_data$train[model_data$train$cell_name==cn, response_variable])
#   })
# 
# round(perf,3)
...
bstDense <- xgboost(data = dtrain, objective = "reg:squarederror", verbose = T, nrounds = 20, print_every_n = 50)
...
differential_single_boost_results <- sapply(cell_names, function(cn)
{
...
# differential_single_boost_results <- sapply(cell_names, function(cn)
# {
#   v <- c(predictors, identifier, response_variable)
#   v2 <- c(identifier, response_variable)
# model_data <- merge(data_tables[[cn]][,v], data_tables[["H1"]][,v2], by = identifier)
# y <- paste(response_variable,".x",sep="")
# predictors <- colnames(model_data)[! colnames(model_data) %in% c(y,identifier)]
# 
# model_data <- model_data[complete.cases(model_data),]
# model_data <- train_and_test_split(model_data,0.8,T)
# 
# dtrain <- xgb.DMatrix(data = as.matrix(model_data$train[, predictors]), label=as.matrix(model_data$train[, y]))
# 
# dtest <- xgb.DMatrix(data = as.matrix(model_data$test[, predictors]), label=as.matrix(model_data$test[, y]))
# 
# mod <- xgboost(data = dtrain, objective = "reg:squarederror", verbose = T, nrounds = 10, print_every_n = 50)
# 
# r <- cor(predict(mod, dtest), model_data$test[,y])
# 
# names(r) <- cn
# r
# }, USE.NAMES = T)
# 
# print(round(differential_single_boost_results,2))
# print(round(differential_single_boost_results^2,2))
...
######### one xgboost for each cell differences
# predictors <- c("log_nucleotides", "CG_density", "meth_rate", "meth_autocorrelation", "mean_bernoulli_entropy", "meth_sd")
# differential_single_linear_results <- sapply(cell_names, function(cn)
# {
#   v <- c(predictors, identifier, response_variable)
#   v2 <- c(identifier, response_variable)
# model_data <- merge(data_tables[[cn]][,v], data_tables[["H1"]][,v2], by = identifier)
...
# 
# 
# mod <- lm(data = model_data$train, formula = model_this(y, predictors))
# r <- prediction_correlation_score(mod, model_data$test)
# names(r) <- cn
# r
# }, USE.NAMES = T)
# 
