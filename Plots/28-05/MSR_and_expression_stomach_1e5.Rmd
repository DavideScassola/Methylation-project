---
title: "MSR and expression, for stomach cells and window size of 100,000"
...
(ggplot(data_table,aes(x=inverted_msr,y=log_tpm))) + geom_point(alpha = 0.5) + ggtitle("inverted msr vs log(tpm)")
```
...
Negative binomial for gene number with basic predictors (nucleotides, CpG_density, meth rate):
...
glm.fit <- glm.nb(model_data$gene_count ~ nucleotides+CpG_density+`meth rate`, data = model_data)
