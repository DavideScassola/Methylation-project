...
```{r, echo=FALSE, warning=FALSE}
((ggplot(data_table,aes(x=meth_autocorrelation,y=log_tpm)) + geom_point(alpha = 0.03)))+ ylab("log (TPM)")
```
...
```{r, echo=FALSE}
#
# glm.fit <- glm.nb(train_model_data$gene_count ~ . , data = train_model_data[, c("gene_count",basic_predictors)])
# tmse(glm.fit, test_model_data, "gene_count")
# 
# glm.fit <- glm.nb(train_model_data$gene_count ~ . , data = train_model_data[, c("gene_count","residual")])
# tmse(glm.fit, test_model_data, "gene_count")
# 
# glm.fit <- glm.nb(train_model_data$gene_count ~ . , data = train_model_data[, c("gene_count",basic_predictors, "residual")])
# tmse(glm.fit, test_model_data, "gene_count")
# 
# 
# glm.fit <- glm.nb(train_model_data$gene_count ~ . , data = train_model_data[, c("gene_count",basic_predictors, msr_predictors)])
# tmse(glm.fit, test_model_data, "gene_count")
