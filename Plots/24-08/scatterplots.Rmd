...
gn <- data_table$gene_count
x <- data_table$`meth rate`
y <- data_table$meth_autocorrelation
ltpm <- log(data_table$total_TPM+1)
group <- as.factor(data_table$total_TPM>1)
df <- na.omit(data.frame(x,y,group, ltpm, gn))
#df <- df[df$gn>0,]
```
